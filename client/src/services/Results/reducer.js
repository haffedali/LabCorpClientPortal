...
export default function resultsReducer(state = {}, action){
  ...
  switch (action.type){
    case SEARCH:
      return {...state, search: action.payLoad, rows: matchedRows(state.rows, action.payLoad)}
    case LOAD_PENDING:
      return {...state, ajaxData: action.payLoad.data, ajaxStatus: action.payLoad.status,}
    case LOAD_SUCCESS:
      return {...state, ajaxData: action.payLoad.data, ajaxStatus: action.payLoad.status, rows: action.payLoad.rows,}
    case LOAD_FAIL:
      return {...state, ajaxData: action.payLoad.data, ajaxStatus: action.payLoad.status,}
    default:
      return state
